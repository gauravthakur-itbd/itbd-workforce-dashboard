    ...
    if pd.notna(email) and pd.notna(name):
        ...
        if email_str not in engineer_database:
            engineer_database[email_str] = {
                ...
                'email': email_str,
                'ttl': str(ttl) if pd.notna(ttl) else 'Unassigned',
                'partners': set(),
                'utilization_records': [],
...
for partner, data in partner_database.items():
    ...
    # Get unique TTLs
    ttls = list(set([e['ttl'] for e in engineers_list]))
...
for i, (partner, data) in enumerate(sorted_partners[:10], 1):
    print(f"{i}. {partner}: {data['engineer_count']} engineers, CSAT: {data['csat_score']}%, TTLs: {', '.join(data['ttls'][:3])}")
